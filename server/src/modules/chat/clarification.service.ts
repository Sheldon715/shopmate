...
const BROAD_CATEGORY_RULES = [
  ...
  {
    terms: ["鞋", "跑鞋", "跑步鞋", "运动鞋", "训练鞋", "慢跑鞋", "竞速鞋", "鞋子"],
...
const RECOMMENDATION_INTENT_PATTERN =
  /推荐|求推荐|有什么|有啥|有没有|想买|买一|买个|帮我|找一|看看|选一|来一|哪个好|哪款/u;
...
const USE_CASE_PATTERN =
  /通勤|上班|学习|办公|游戏|拍照|旅行|出差|宿舍|跑步|慢跑|健身|训练|竞速|马拉松|户外|日常|夏天|冬天|送礼|约会|一人食|租房/u;
...
function hasEnoughContext(
  input: ClarificationServiceInput & {
    broadCategoryTerms: readonly string[];
  },
): boolean {
  const constraints = input.contextMemory?.constraints;
  const filters = input.filters;
  const constraintQuestion = removeBroadCategoryTerms(
    input.question,
    input.broadCategoryTerms,
  );
...
function removeBroadCategoryTerms(
  question: string,
  terms: readonly string[],
): string {
  return [...terms]
    .sort((left, right) => right.length - left.length)
    .reduce((currentQuestion, term) => currentQuestion.split(term).join(""), question);
}
