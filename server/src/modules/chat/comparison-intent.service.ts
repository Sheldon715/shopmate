...
export class ComparisonIntentService {
  ...
  async detect(
    ...
    try {
      ...
      const focusedParsed = hasRecentRecommendationComparisonCue(input)
        ? await this.detectRecentRecommendationComparison(input)
        : undefined;
  ...
  private async detectRecentRecommendationComparison(
    input: ComparisonIntentDetectInput,
  ): Promise<ComparisonIntentResult | undefined> {
    try {
      const response = await this.llmClient.generate({
        messages: buildRecentRecommendationComparisonPrompt(input),
        temperature: 0,
        maxCompletionTokens: COMPARISON_INTENT_MAX_COMPLETION_TOKENS,
        requestId: input.requestId,
        abortSignal: input.abortSignal,
    ...
    },
    {
      ...
      content: JSON.stringify({
        ...
        explicitRecentOrdinalPair:
          extractExplicitRecentOrdinalPair(input.question) ?? [],
      }),
    },
  ];
}

function buildRecentRecommendationComparisonPrompt(
  input: ComparisonIntentDetectInput,
): LlmGenerateRequest["messages"] {
  return [
    {
      ...
      content: [
        ...
        "代码只提供候选线索，最终仍由你判断用户是否在要求比较最近推荐商品。",
        "如果用户当前消息是在问最近推荐中的多款商品哪个更适合、怎么选、有什么差异，输出 is_comparison=true。",
        "如果用户是在要求推荐新商品、加入购物车、删除购物车或普通闲聊，输出 is_comparison=false。",
        "当 is_comparison=true 时，target.kind 必须是 recent_recommendations；如果没有明确序号但说“这两款/这两个”，ordinals 输出 [1,2]。",
...
export function parseComparisonIntentOutput(
  ...
  return {
    ...
    clarificationQuestion: exceedsSupportedProductCount
...
function createExplicitRecentOrdinalComparisonIntent(
  ...
  const ordinals = extractExplicitRecentOrdinalPair(normalized);
